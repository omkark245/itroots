...
function parsePathToVertices(path: string, sampleLength = 15) {
    ...
    const totalLength = (commander as any).getTotalLength();
    let length = 0;

    // Sample points along the path
    while (length < totalLength) {
        const point = (commander as any).getPointAtLength(length);
    ...
    const finalPoint = (commander as any).getPointAtLength(totalLength);
    ...
    (
        ...
        const registerElement = useCallback(
            (id: string, element: HTMLElement, props: MatterBodyProps) => {
                ...
                let body
                const { chamfer, ...otherOptions } = props.matterBodyOptions || {};
                const cleanOptions = {
                    ...otherOptions,
                    ...(chamfer ? { chamfer } : {})
                };
